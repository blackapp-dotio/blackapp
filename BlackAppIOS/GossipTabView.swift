...
// === Cached remote image (for full HTTPS URLs) ===============================
...
struct AvatarView: View {
    ...
    var body: some View {
        ...
        .task(id: urlString ?? "") {
            ...
            // Uses the ImageStore from PerformanceKit (mem+disk cache)
    ...
    struct GossipTabView: View {
        ...
        // Optional proxy (unused but kept)
        ...
        // MARK: Sources — nightlife-leaning (~80% nightlife, ~20% news)
        ...
        var body: some View {
            VStack(spacing: 0) {
                ...
                VStack(spacing: 8) {
                    ...
                    // Action row (Picker • Discard • Send)
                    HStack(spacing: 16) {
                        // open media picker
                ...
                .padding(.horizontal)
                .padding(.top, 8)
                // Replace your ambiguous .toolbar block with this:
                .toolbar {
            ...
            .onAppear {
                // Instant paint from cache, then refresh network content
                loadCache()
                reloadContent()
            }
            .onChange(of: selectedVideoURL) { newValue in
                // Keep a strong player ref so VideoPlayer doesn’t show a black frame
                if let url = newValue {
            ...
            // 🔊 Global listener from LiveCaptureView → routes into composer
        ...
        /// Handle media passed from LiveCaptureView (photo or video).
                ...
                .map { post in
                    ...
                    return AnyIdentifiablePost(timestamp: post.timestamp, id: post.id) {
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            // Optional user media (dynamic fit for image & video)
                            if let mediaURL = post.mediaURL, let url = URL(string: mediaURL) {
                                Group {
                                    if post.mediaType == "video" {
                                        // Dynamically sizes to the actual aspect ratio of the video
                                        DynamicVideoPlayer(url: url)
                                    } else {
                                        // Dynamically sizes to the actual aspect ratio of the image (no crop)
        ...
        private func loadMoreIfNeeded() {
            ...
            throttler.throttle("loadMore", interval: 0.35) {
                loadingMore = true
                // Small delay to let the last cell settle before growing the page
    ...
    // iOS 15+ keyWindow helper
    private extension UIWindowScene {
        var keyWindow: UIWindow? { windows.first { $0.isKeyWindow } }
    }
    
    // MARK: - WebView used by your sheet
